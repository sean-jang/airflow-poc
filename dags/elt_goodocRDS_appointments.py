...
from tools.transform import AppointmentsTransformer
from tools.load import S3Loader


with DAG(
    dag_id="elt_goodocRDS_appointments",
    schedule="10 17 * * *",
    ...
    bronze = SqlToS3Operator(
        ...
        query="SELECT * FROM appointments WHERE SUBSTR(DATE_ADD(createdAt, interval 9 hour), 1, 10) = '{{ next_ds }}'",
        s3_bucket="goodoc-sync-rds-data",
        s3_key="bronze/goodoc-rds/appointments/date_id={{ next_ds }}/0.csv",
        replace=True,
    ...
    bronze_sensor = S3KeySensor(
        ...
        bucket_key="bronze/goodoc-rds/appointments/date_id={{ next_ds }}/0.csv",
    ...
    extract_bronze = PythonOperator(
        ...
        op_kwargs={
            ...
            "key": "bronze/goodoc-rds/appointments/date_id={{ next_ds }}/0.csv"
        }
    )

    transform_to_gold = PythonOperator(
        task_id="transform_to_gold",
        python_callable=AppointmentsTransformer.transform,
        op_kwargs={
    ...
    load = PythonOperator(
        ...
        op_kwargs={
            ...
            "bucket": "goodoc-sync-rds-data",
            "key": "gold/daily_key_metrics/date_id={{ next_ds }}/appointments_summary.csv",
        }
    )
